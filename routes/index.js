    ...
    if (process.env.SLACK_TOKEN == req.body.token) {
        ...
        if (req.body.text.match(/logout/i)) { 
            Model.User.remove({
                ...
                if (!err) {
                    thisResponse.text = 'We will miss you ' + name;
                    thisResponse.attachments[0].color = 'good';
                    thisResponse.attachments[0].text = 'Hooman has been removed';
                    res.send(thisResponse);
                } else {
                    thisResponse.attachments[0].text = 'The gods don\'t wanna leave you hooman';
                    res.send(thisResponse);
                }
            });
        } else {
